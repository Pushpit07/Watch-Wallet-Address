...
export default function InputForm ({getTransactions, setAccount}) {
    ...
    function chainChanged(event) {
        setChain(event.target[event.target.selectedIndex].dataset.chainlookupvalue);
    }
    ...
    return (
        <>
            <div className="w-2/3 grid grid-cols-5 gap-4">
                <label htmlFor="address" className="mb-1 block text-sm font-medium text-gray-700 text-left">
                    Wallet address
                </label>
            </div>
            <form onSubmit={(event) => {event.preventDefault(); getTransactions(chain);}} className="w-2/3 grid grid-cols-5 gap-4">
                ...
                <input type="submit" className="self-end h-full cursor-pointer bg-white hover:bg-gray-100 text-gray-600 font-semibold border border-gray-300 rounded" />
            </form>	
        </>
    );
}
